...
class BahiaBlancaPort:
    ...
    def build_invoice_map(self, analysis, work_dir, line_amounts):
        ...
        entries   = {}
        entries["AGENCY FEE"] = {"concept":"AGENCY FEE","amount":next((f.get("total",0) for f in analysis["facbs"] if f.get("type")=="agency"),0),"tc":tc_agency,"invoices":[],"solo":True}
        if analysis.get("consorcio"):
            entries["PORT DUES"] = {"concept":"PORT DUES","amount":amt("PORT DUES"),"tc":tc_port,"invoices":[(analysis["consorcio"][0],None)]}
            entries["TOLL DUES"] = {"concept":"TOLL DUES","amount":amt("TOLL DUES"),"tc":tc_port,"invoices":[(f,None) for f in analysis["consorcio"]]}
        if analysis.get("donmar"):
            entries["PORT PILOTAGE"] = {"concept":"PORT PILOTAGE","amount":amt("PORT PILOTAGE"),"tc":tc_port,"invoices":[(f,None) for f in analysis["donmar"]]}
        ...
        if mooring and amt("MOORING & UNMOORING SERVICES") > 0:
            entries["MOORING & UNMOORING SERVICES"] = {"concept":"MOORING & UNMOORING SERVICES","amount":amt("MOORING & UNMOORING SERVICES"),"tc":tc_port,"invoices":mooring}
        if analysis.get("puerto_mariel") and amt("TOWAGE SERVICES") > 0:
            entries["TOWAGE SERVICES"] = {"concept":"TOWAGE SERVICES","amount":amt("TOWAGE SERVICES"),"tc":tc_port,"invoices":[(f,None) for f in analysis["puerto_mariel"]]}
        for v in ["CUSTOM HOUSE EXPENSES","CUSTOM HOUSE PERMANENCE","CUSTOM HOUSE (BUNKERING)","MIGRATION EXPENSES","SANITARY DUES AND FREE PRATIQUE","GARBAGE COMPULSORY INSPECTION","WATCHMEN COMPULSORY SERVICES","HEADCLERK COMPULSORY SERVICES"]:
            inv = mar.get(v,[])
        ...
        entries["TAX ON CREDIT/DEBIT LAW 25.413"] = {"concept":"TAX ON CREDIT/DEBIT LAW 25.413","amount":amt("TAX ON CREDIT/DEBIT LAW 25.413"),"tc":tc_port,"invoices":[],"solo":True}
...
class NecocheaPort:
    ...
    def build_invoice_map(self, analysis, work_dir, line_amounts):
        ...
        if consorcio:
            entries["PORT DUES"] = {"concept":"PORT DUES","amount":amt("PORT DUES"),"tc":tc_port,"invoices":[(consorcio[0],None)]}
        if len(consorcio) >= 2:
            entries["ENTRANCE AND LIGHT DUES"] = {"concept":"ENTRANCE AND LIGHT DUES","amount":amt("ENTRANCE AND LIGHT DUES"),"tc":tc_port,"invoices":[(consorcio[1],None)]}
        elif consorcio:
            entries["ENTRANCE AND LIGHT DUES"] = {"concept":"ENTRANCE AND LIGHT DUES","amount":amt("ENTRANCE AND LIGHT DUES"),"tc":tc_port,"invoices":[(consorcio[0],None)]}
        toll_inv = consorcio[2:] if len(consorcio) > 2 else consorcio
        if toll_inv:
            entries["TOLL DUES"] = {"concept":"TOLL DUES","amount":amt("TOLL DUES"),"tc":tc_port,"invoices":[(f,None) for f in toll_inv]}
        pilotaje = [(f,None) for f in analysis.get("pilotaje",[])] + mar.get("PORT PILOTAGE",[])
        if pilotaje:
            entries["PORT PILOTAGE"] = {"concept":"PORT PILOTAGE","amount":amt("PORT PILOTAGE"),"tc":tc_port,"invoices":pilotaje}
        melluso = [(f,None) for f in analysis.get("melluso",[])] + mar.get("MOORING & UNMOORING SERVICES",[])
        if melluso:
            entries["MOORING & UNMOORING SERVICES"] = {"concept":"MOORING & UNMOORING SERVICES","amount":amt("MOORING & UNMOORING SERVICES"),"tc":tc_port,"invoices":melluso}
        sg = [(f,None) for f in analysis.get("shore_gangway",[])] + mar.get("SHORE GANGWAY",[])
        if sg:
            entries["SHORE GANGWAY"] = {"concept":"SHORE GANGWAY","amount":amt("SHORE GANGWAY"),"tc":tc_port,"invoices":sg}
        if analysis.get("puerto_mariel"):
            entries["TOWAGE SERVICES"] = {"concept":"TOWAGE SERVICES","amount":amt("TOWAGE SERVICES"),"tc":tc_port,"invoices":[(f,None) for f in analysis["puerto_mariel"]]}
        ch = mar.get("CUSTOM HOUSE EXPENSES",[]) + [(f,None) for f in analysis.get("centro_nav",[])]
        if ch:
            entries["CUSTOM HOUSE EXPENSES"] = {"concept":"CUSTOM HOUSE EXPENSES","amount":amt("CUSTOM HOUSE EXPENSES"),"tc":tc_port,"invoices":ch}
        for v in ["CUSTOM HOUSE PERMANENCE","MIGRATION EXPENSES","SANITARY DUES AND FREE PRATIQUE","GARBAGE COMPULSORY INSPECTION","WATCHMEN COMPULSORY SERVICES","HEADCLERK COMPULSORY SERVICES"]:
            inv = mar.get(v,[])
            if inv and amt(v) > 0: entries[v] = {"concept":v,"amount":amt(v),"tc":tc_port,"invoices":inv}
...
class SanLorenzoPort:
    ...
    VOUCHER_ORDER = [
        ...
        "FULL ON HIRE / BQS SURVEY","BQS EXPENSES","GAS FREE INSPECTION","PEST CONTROL",
        "TAX ON CREDIT/DEBIT LAW 25.413",
        "TOLL DUES (CARP)","PILOT LAUNCH TRANSPORTATION RIVER PLATE","TOLL DUES (AGP)",
    ]
    ...
    def build_invoice_map(self, analysis, work_dir, line_amounts):
        ...
        # Agency Fee — solo voucher
        entries["AGENCY FEE"] = {"concept":"AGENCY FEE","amount":next((f.get("total",0) for f in analysis["facbs"] if f.get("type")=="agency"),0),"tc":tc_agency,"invoices":[],"solo":True}

        # Port Dues
        term = [(f,None) for f in analysis.get("terminal_portuario",[])]
        if term and amt("PORT DUES") > 0:
            entries["PORT DUES"] = {"concept":"PORT DUES","amount":amt("PORT DUES"),"tc":tc_port,"invoices":term}

        # Entrance and Light Dues — ENAPRO (página Maritime)
        enapro = mar.get("ENTRANCE AND LIGHT DUES",[])
        if enapro and amt("ENTRANCE AND LIGHT DUES") > 0:
            entries["ENTRANCE AND LIGHT DUES"] = {"concept":"ENTRANCE AND LIGHT DUES","amount":amt("ENTRANCE AND LIGHT DUES"),"tc":tc_port,"invoices":enapro}
        ...
        if rp_base and amt("RIVER PLATE PILOTAGE") > 0:
            entries["RIVER PLATE PILOTAGE"] = {"concept":"RIVER PLATE PILOTAGE","amount":amt("RIVER PLATE PILOTAGE"),"tc":tc_port,"invoices":rp_base}
        if rp_delay and amt("RIVER PLATE PILOTAGE (DELAY)") > 0:
            entries["RIVER PLATE PILOTAGE (DELAY)"] = {"concept":"RIVER PLATE PILOTAGE (DELAY)","amount":amt("RIVER PLATE PILOTAGE (DELAY)"),"tc":tc_port,"invoices":rp_delay}
        if rp_manio and rp_manio_amt > 0:
            entries["RIVER PLATE PILOTAGE ANCHORAGE MANEUVER"] = {"concept":"RIVER PLATE PILOTAGE ANCHORAGE MANEUVER","amount":rp_manio_amt,"tc":tc_port,"invoices":rp_manio}

        # River Parana Pilotage — COPRAC: solo p1 de cada factura ([0])
        cp_all  = analysis.get("coprac",[])
        cp_base = [(r["filename"],[0]) for r in cp_all]
        ...
        if cp_base and amt("RIVER PARANA PILOTAGE") > 0:
            entries["RIVER PARANA PILOTAGE"] = {"concept":"RIVER PARANA PILOTAGE","amount":amt("RIVER PARANA PILOTAGE"),"tc":tc_port,"invoices":cp_base}
        if cp_delay and amt("RIVER PARANA PILOTAGE (DELAY)") > 0:
            entries["RIVER PARANA PILOTAGE (DELAY)"] = {"concept":"RIVER PARANA PILOTAGE (DELAY)","amount":amt("RIVER PARANA PILOTAGE (DELAY)"),"tc":tc_port,"invoices":cp_delay}
        if cp_manio and cp_manio_amt > 0:
            entries["RIVER PARANA PILOTAGE ANCHORAGE MANEUVER"] = {"concept":"RIVER PARANA PILOTAGE ANCHORAGE MANEUVER","amount":cp_manio_amt,"tc":tc_port,"invoices":cp_manio}

        # Port Pilotage — Rosario Pilots: todas las páginas (facturas + vouchers internos)
        ...
        if rsp_base and amt("PORT PILOTAGE") > 0:
            entries["PORT PILOTAGE"] = {"concept":"PORT PILOTAGE","amount":amt("PORT PILOTAGE"),"tc":tc_port,"invoices":rsp_base}
        if rsp_delay and amt("PORT PILOTAGE (DELAY)") > 0:
            entries["PORT PILOTAGE (DELAY)"] = {"concept":"PORT PILOTAGE (DELAY)","amount":amt("PORT PILOTAGE (DELAY)"),"tc":tc_port,"invoices":rsp_delay}

        # Launch Services for Clearance — Amarre Coral clearance: solo original (p1=[0])
        ...
        if clearance_inv and amt("LAUNCH SERVICES FOR CLEARANCE (AT ROADS)") > 0:
            entries["LAUNCH SERVICES FOR CLEARANCE (AT ROADS)"] = {"concept":"LAUNCH SERVICES FOR CLEARANCE (AT ROADS)","amount":amt("LAUNCH SERVICES FOR CLEARANCE (AT ROADS)"),"tc":tc_port,"invoices":clearance_inv}
        ...
        if all_mooring and amt("MOORING & UNMOORING SERVICES") > 0:
            entries["MOORING & UNMOORING SERVICES"] = {"concept":"MOORING & UNMOORING SERVICES","amount":amt("MOORING & UNMOORING SERVICES"),"tc":tc_port,"invoices":all_mooring}

        # Maritime vouchers
        for voucher in ["CUSTOM HOUSE EXPENSES","CUSTOM HOUSE PERMANENCE","CUSTOM HOUSE EXPENSE (CARGO)",
                        "MIGRATION EXPENSES","SANITARY DUES AND FREE PRATIQUE","GARBAGE COMPULSORY INSPECTION",
                        "HEADCLERK COMPULSORY SERVICES","WATCHMEN COMPULSORY SERVICES","PEST CONTROL","OSRO ANNEX 18"]:
            inv = mar.get(voucher,[])
            if inv and amt(voucher) > 0:
                entries[voucher] = {"concept":voucher,"amount":amt(voucher),"tc":tc_port,"invoices":inv}
        ...
        if mand_insp and amt("MANDATORY HOLDS INSPECTION") > 0:
            entries["MANDATORY HOLDS INSPECTION"] = {"concept":"MANDATORY HOLDS INSPECTION","amount":amt("MANDATORY HOLDS INSPECTION"),"tc":tc_port,"invoices":mand_insp}
        if mand_reinsp and amt("MANDATORY HOLDS RE-INSPECTION") > 0:
            entries["MANDATORY HOLDS RE-INSPECTION"] = {"concept":"MANDATORY HOLDS RE-INSPECTION","amount":amt("MANDATORY HOLDS RE-INSPECTION"),"tc":tc_port,"invoices":mand_reinsp}

        # Full On Hire / BQS Survey — EDI Separovic: solo original (p1=[0])
        survey_inv = [(f,[0]) for f in analysis.get("edi_separovic",[])]
        if survey_inv and amt("FULL ON HIRE / BQS SURVEY") > 0:
            entries["FULL ON HIRE / BQS SURVEY"] = {"concept":"FULL ON HIRE / BQS SURVEY","amount":amt("FULL ON HIRE / BQS SURVEY"),"tc":tc_port,"invoices":survey_inv}
        ...
        if all_nav and amt("NAVIGATION CENTER CONTRIBUTION") > 0:
            entries["NAVIGATION CENTER CONTRIBUTION"] = {"concept":"NAVIGATION CENTER CONTRIBUTION","amount":amt("NAVIGATION CENTER CONTRIBUTION"),"tc":tc_port,"invoices":all_nav}

        # Tax on Credit/Debit Law 25.413
        tax_amt = amt("TAX ON CREDIT/DEBIT LAW 25.413")
        if tax_amt > 0:
            entries["TAX ON CREDIT/DEBIT LAW 25.413"] = {"concept":"TAX ON CREDIT/DEBIT LAW 25.413","amount":tax_amt,"tc":tc_port,"invoices":[],"solo":True}

        # Toll Dues CARP — factura CARP SOLO bajo este voucher, nunca en otros
        carp_inv = [(f,None) for f in analysis.get("carp",[])]
        if amt("TOLL DUES (CARP)") > 0:
            entries["TOLL DUES (CARP)"] = {"concept":"TOLL DUES (CARP)","amount":amt("TOLL DUES (CARP)"),"tc":tc_port,"invoices":carp_inv}
        ...
        if glatil_inv and amt("PILOT LAUNCH TRANSPORTATION RIVER PLATE") > 0:
            entries["PILOT LAUNCH TRANSPORTATION RIVER PLATE"] = {"concept":"PILOT LAUNCH TRANSPORTATION RIVER PLATE","amount":amt("PILOT LAUNCH TRANSPORTATION RIVER PLATE"),"tc":tc_port,"invoices":glatil_inv}

        # Toll Dues AGP — facturas AGP SOLO bajo este voucher
        agp_inv = [(f,None) for f in analysis.get("agp",[])]
        if agp_inv and amt("TOLL DUES (AGP)") > 0:
            entries["TOLL DUES (AGP)"] = {"concept":"TOLL DUES (AGP)","amount":amt("TOLL DUES (AGP)"),"tc":tc_port,"invoices":agp_inv}
